...
template <typename TYPE1, typename TYPE2> TYPE2 * cuozblasRcsrmvSplitA (cuozblasHandle_t *oh, const char tranA, const int32_t m, const int32_t n, const int32_t nnz, const cusparseMatDescr_t descrA, const TYPE1 *devA, const int32_t *devArowptr);//, TYPE2 *devASplit);
