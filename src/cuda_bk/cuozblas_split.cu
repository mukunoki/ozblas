...
void cuozblasSplitCKernel (
	...
	if (addry < n){
		...
		printf ("sigma=%1.3e\n", sigma);
		//const TYPE sigma = MUL (MUL (MUL (CONST, scalbn (1., rho)), NextPowTwo <TYPE> (devMax[addry])), splitShift);
		//const short tau = ceil(log2(fabs(devMax[addry])));
		//const TYPE sigma = MUL (MUL (CONST, scalbn (1., rho + tau)), splitShift);
		TYPE max = 0.;
		for (int32_t i = addrx; i < m; i += nTx) {
			TYPE input = devInput[addry * ldi + i];
			const TYPE tmp = ADD (input, sigma);
			const TYPE split = SUB (tmp, sigma);
			//const TYPE split = SUB (ADD (input, sigma), sigma);
			printf ("[%d] split=%1.3e input=%1.3e\n", i, split, input);
...
void cuozblasSplitVecKernel (
	...
	//const TYPE sigma = MUL (MUL (MUL (CONST, scalbn (1., rho)), NextPowTwo <TYPE> (devMax[0])), splitShift);
	const short tau = ceil(log2(fabs(devMax[0]))); // note: here, devMax needs fabs
	const TYPE sigma = MUL (MUL (CONST, scalbn (1., rho + tau)), splitShift);
	TYPE max_ = 0.;
...
int32_t cuozblasSplit (
	...
	cudaMemcpy (hst, devInput, sizeof(TYPE1)*m, cudaMemcpyDeviceToHost); 
	printf ("devInput: ");
	for (int i = 0; i < m; i++)
		printf ("%1.2e ", hst[i]);
	printf ("\n");
	TYPE1 tmp;
	cudaMemcpy (&tmp, devMax, sizeof(TYPE1), cudaMemcpyDeviceToHost); 
	printf ("devMax=%1.3e\n", tmp);

	// Split^(0) & FindMax^(1)
	//printf ("splitEps=%d, splitShift=%d, ldi=%d, ldo=%d, lds=%d\n", oh->splitEpsModeFlag, oh->splitShift, ldi, ldo, lds);
	...
	cudaMemcpy (hst, devOutput, sizeof(TYPE1)*m, cudaMemcpyDeviceToHost); 
	printf ("devInput: ");
	for (int i = 0; i < m; i++)
		printf ("%1.2e ", hst[i]);
	printf ("\n");
...
void cuozblasSplitSparseNKernel (
	...
	if (rowid < m){
		...
		//const TYPE sigma = MUL (MUL (CONST, scalbn (1., rho)), NextPowTwo <TYPE> (devMax[rowid])) / splitShift;
		const short tau = ceil(log2(devMax[rowid]));
		const TYPE sigma = MUL (CONST, scalbn (1., rho+tau)) / splitShift;
		TYPE max = 0.;
		for (int32_t i = devRowptr[rowid] + lane; i < devRowptr[rowid+1]; i += 32) {
			TYPE input = devInput[i];
			const TYPE split = SUB (ADD (input, sigma), sigma);
...
void cuozblasSplitSparseNKernel (
	...
	if (rowid < m){
		...
		const TYPE1 sigma = MUL (CONST, scalbn (1., rho+tau)) / splitShift;
		TYPE1 max = 0.;
		for (int32_t i = devRowptr[rowid] + lane; i < devRowptr[rowid+1]; i += 32) {
			TYPE1 input = devInput[i];
			const TYPE1 split = SUB (ADD (input, sigma), sigma);
